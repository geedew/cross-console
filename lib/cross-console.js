;(function(window, undefined) {
    ...
    CC.settings = {
		...
		history: persist ? JSON.parse(root.localStorage.getItem(CC.settings.persistConfig.history)) || [] : [],
		// limit the size of the array to 100 
		maxHistory: 100,
		filter: persist ? JSON.parse(root.localStorage.getItem(CC.settings.persistConfig.filter)) || false: false,
	...
	updateHistory = function(msg) {
		...
		if( persist ) {
			root.localStorage.setItem(CC.settings.persistConfig.history,root.JSON.stringify(CC.settings.history));
		}
	},
	...
	loadTraceMethods = function(idx,method){
		CC[method] = function() {
			...
			try {
				...
				if ( !con || ( alwaysShow.indexOf(method) < 0 && (!CC.settings.debug || args[0].indexOf(CC.settings.filter) < 0   ))) { 
					return;
				}
